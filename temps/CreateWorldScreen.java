...
public class CreateWorldScreen extends Screen {
    ...
    public static CreateWorldScreen create(MinecraftClient client, @Nullable Screen parent, LevelInfo levelInfo, GeneratorOptionsHolder generatorOptionsHolder, @Nullable Path dataPackTempDir) {
        ...
        if (levelInfo.isHardcore()) {
            createWorldScreen.worldCreator.setGameMode(Mode.HARDCORE);
        } else if (levelInfo.getGameMode().isSurvivalLike()) {
            createWorldScreen.worldCreator.setGameMode(Mode.SURVIVAL);
        } else if (levelInfo.getGameMode().isCreative()) {
            createWorldScreen.worldCreator.setGameMode(Mode.CREATIVE);
        }
    ...
    protected void init() {
        this.tabNavigation = TabNavigationWidget.builder(this.tabManager, this.width).tabs(new Tab[]{new GameTab(), new WorldTab(), new MoreTab()}).build();
    ...
    protected <T extends Element & Selectable> T addSelectableChild(T child) {
        return (T)super.addSelectableChild(child);
    }

    protected <T extends Element & Drawable & Selectable> T addDrawableChild(T drawableElement) {
        return (T)super.addDrawableChild(drawableElement);
    }
    ...
    class GameTab extends GridScreenTab {
        ...
        GameTab() {
            ...
            CyclingButtonWidget<WorldCreator.Mode> cyclingButtonWidget = (CyclingButtonWidget)adder.add(CyclingButtonWidget.builder((value) -> value.name).values(new WorldCreator.Mode[]{Mode.SURVIVAL, Mode.HARDCORE, Mode.CREATIVE}).build(0, 0, 210, 20, CreateWorldScreen.GAME_MODE_TEXT, (button, value) -> CreateWorldScreen.this.worldCreator.setGameMode(value)), positioner);
            ...
            CyclingButtonWidget<Difficulty> cyclingButtonWidget2 = (CyclingButtonWidget)adder.add(CyclingButtonWidget.builder(Difficulty::getTranslatableName).values(Difficulty.values()).build(0, 0, 210, 20, Text.translatable("options.difficulty"), (button, value) -> CreateWorldScreen.this.worldCreator.setDifficulty(value)), positioner);
            CreateWorldScreen.this.worldCreator.addListener((creator) -> {
