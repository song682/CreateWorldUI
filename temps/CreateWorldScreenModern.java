...
public class CreateWorldScreen extends Screen {
    ...
    public static void testWorld(final Minecraft minecraft, final Runnable onClose) {
        WorldCreationContextMapper worldCreationContext = (managers, registries, cookie) -> new WorldCreationContext(cookie.worldGenSettings().options(), cookie.worldGenSettings().dimensions(), registries, managers, cookie.dataConfiguration(), new InitialWorldCreationOptions(SelectedGameMode.CREATIVE, (new GameRuleMap.Builder()).set(GameRules.ADVANCE_TIME, false).set(GameRules.ADVANCE_WEATHER, false).set(GameRules.SPAWN_MOBS, false).build(), FlatLevelGeneratorPresets.REDSTONE_READY));
    ...
    public static CreateWorldScreen createFromExisting(final Minecraft minecraft, final Runnable onClose, final LevelSettings levelSettings, final WorldCreationContext worldCreationContext, final @Nullable Path newDataPackDir) {
        ...
        result.uiState.getGameRules().setAll(worldCreationContext.initialWorldCreationOptions().gameRuleOverwrites(), (MinecraftServer)null);
        if (levelSettings.difficultySettings().hardcore()) {
            result.uiState.setGameMode(SelectedGameMode.HARDCORE);
        } else if (levelSettings.gameType().isSurvival()) {
            result.uiState.setGameMode(SelectedGameMode.SURVIVAL);
        } else if (levelSettings.gameType().isCreative()) {
            result.uiState.setGameMode(SelectedGameMode.CREATIVE);
        }
    ...
    protected void init() {
        this.tabNavigationBar = TabNavigationBar.builder(this.tabManager, this.width).addTabs(new Tab[]{new GameTab(), new WorldTab(), new MoreTab()}).build();
    ...
    private void onCreate() {
        ...
        LayeredRegistryAccess<RegistryLayer> finalLayers = context.worldgenRegistries().replaceFrom(RegistryLayer.DIMENSIONS, new RegistryAccess.Frozen[]{finalDimensions.dimensionsRegistryAccess()});
    ...
    private static WorldLoader.InitConfig createDefaultLoadConfig(final PackRepository packRepository, final WorldDataConfiguration config) {
        ...
        return new WorldLoader.InitConfig(packConfig, CommandSelection.INTEGRATED, LevelBasedPermissionSet.GAMEMASTER);
    }
    ...
    private class GameTab extends GridLayoutTab {
        ...
        private GameTab() {
            ...
            CreateWorldScreen.this.uiState.addListener((uiState) -> this.nameEdit.setTooltip(Tooltip.create(Component.translatable("selectWorld.targetFolder", new Object[]{Component.literal(uiState.getTargetFolder()).withStyle(ChatFormatting.ITALIC)}))));
    ...
    private class WorldTab extends GridLayoutTab {
        ...
        private static MutableComponent createTypeButtonNarration(final CycleButton<WorldCreationUiState.WorldTypeEntry> button) {
            return ((WorldCreationUiState.WorldTypeEntry)button.getValue()).isAmplified() ? CommonComponents.joinForNarration(new Component[]{button.createDefaultNarrationMessage(), AMPLIFIED_HELP_TEXT}) : button.createDefaultNarrationMessage();
        }
    }
